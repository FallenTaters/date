...
func (d Date[F]) GoString() string {
	return fmt.Sprintf(`format.NewDate[%T](%d, %d, %d)`, *new(F), d.t.Year(), d.t.Month(), d.t.Day())
}
...
func (d *Date[F]) Scan(src any) error {
	...
	case time.Time:
		*d = DateFrom[F](v)
	default:
		return fmt.Errorf(`%w: cannot unmarshal variable of type %T into format.Date`, ErrScan, src)
	}

	if err != nil {
		return fmt.Errorf(`%w: unable to unmarshal %v into format.Date[%T]: %s`, ErrScan, src, *new(F), err.Error())
	}
